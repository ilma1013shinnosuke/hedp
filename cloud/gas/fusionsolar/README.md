...
キュー形式はschema version 2とする。GASが`JSON.stringify`した完全な応答文字列を
`payload_text`へ保存し、そのUTF-8バイト列のSHA-256を記録する。Mac側でJSONを再整形して
hashを作り直さないため、日本語や数値表現が変わっても改ざん検知がずれない。

## Macへの安全な取込

Driveから専用のローカルフォルダへJSONをダウンロードした後、最初にDBを開かない検査を行う。

```console
hedp import-fusionsolar-gas-queue /path/to/downloaded-queue --inspect
```

次に`.env`を実行時だけ読み、読取専用DBと比較する。

```console
hedp import-fusionsolar-gas-queue /path/to/downloaded-queue --dry-run
```

実取込は明示的にオプションを外した場合だけ行う。同一内容は飛ばし、同じsource・対象日に
異なる内容があれば停止する。RawDataと受領記録は同じトランザクションで保存し、入力JSONは
削除・移動・書換えない。検査結果にはファイル名、source、対象日、hashだけを出し、payloadや
認証情報は出さない。
...
- `SUMICORE_AUTH_ALERT_EMAIL`（任意。認証切れ通知を受け取るメールアドレス）
...
認証切れ時は、秘密値を含まない状態・検知時刻・検知種別をScript Propertiesへ記録する。
通知先が設定されている場合だけメールを送り、連続通知は6時間抑制する。通知失敗の詳細や
認証情報はログへ出さず、通知の失敗によって本来の認証エラーを隠さない。
...
コードだけの配備前検査はGoogleや実APIへ接続せず実行できる。

```console
PYTHONPATH=src python scripts/check_release_readiness.py gas cloud/gas/fusionsolar
```

1. Apps Scriptプロジェクトと専用Driveフォルダを作成する。
2. OAuth権限が外部通信、Drive、trigger、メール送信に限定されていることを確認する。
