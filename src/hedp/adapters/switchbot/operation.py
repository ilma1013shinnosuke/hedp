...
the official model-specific vocabulary represented below.
"""
...
class RobotCommandTransport(Protocol):
    """One call causes at most one command dispatch; retry is forbidden."""
    ...
    def execute(self, request: RobotOperationRequest) -> RobotOperationResult:
        ...
        if self._transport is None:
            raise NotImplementedError(
                "no qualified SwitchBot operation transport is installed"
            )
        try:
