...
既定は画面設計を安全に確認するためのShadow表示です。匿名の模擬値だけを返し、
現役DB、実機Adapter、ExecutionGate、外部APIには接続しません。画面上の操作も
実機へ送信しません。

明示したSQLiteを表示する場合だけ、次の読み取り専用経路を使います。

```bash
hedp interface --database /path/to/hedp.db
```

この経路は既存のStorageと標準HTTPサーバーを再利用し、追加パッケージを必要と
しません。確認済みのFusionSolar正規化値だけを投影し、未取得値は`null`、
古い値は`stale`、読み取り失敗は`missing`として項目ごとに表示します。各値には
観測時刻を添え、画面全体の取得不能も警告として区別します。DB、実機、
ExecutionGateへ書き込まず、エラー本文や家庭固有情報を画面へ返しません。
...
ホーム画面には発電電力と蓄電池残量の推移、当日発電量、自家消費率を表示します。
表示期間は「今日」「7日」「30日」から変更できます。期間を広げても当日発電量は
当日の観測だけから選び、過去日の累積値を当日値として表示しません。
Shadow表示は匿名の模擬時系列です。読み取り専用表示はFusionSolarまたはModbusの
正規化済み観測値を読み取り、欠損区間は線で補間せず、欠損として表示します。

## 次の接続順序

1. Shadow表示とレスポンシブ表示を確認
2. 読み取り専用の現在値・時系列取得口を定義（完了）
3. 匿名fixtureで欠損、古い値、異常値を検証（完了）
