...
だが、実データのarchive、compact DB作成、切替、削除は実施していない。コード完成と本番配備を
...
第3層はfixtureだけを使う副作用のない最小判断と説明生成、第4層はmemory内のShadow
ExecutionGateまで実装・test済みである。本番dispatch、永続監査、read-backは未実装であり、
実機へ直接操作する経路はまだない。
...
**実装状況:** Intentの形、鮮度・品質・期限・能力・重複等を検査するmemory内Shadow Gateは
実装・test済みである。永続的な重複防止と監査、対象台帳、メーカー命令への変換、dispatch、
状態再取得、queueは未実装であり、実送信は行わない。
...
**実装状況:** 太陽光自家消費機会について、注入された匿名fixtureから保守的に判断し、
根拠と判断しない理由を返す最小処理は実装・test済みである。現役DBからの入力選択、
利用者へ表示するread-only CLIも実装済みである。提案の承認、Intent生成、自動操作は
未実装である。
